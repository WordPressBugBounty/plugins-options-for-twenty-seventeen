...
if (!class_exists('options_for_twenty_seventeen_class')) {
	...
	class options_for_twenty_seventeen_class {
        ...
        function ofts_customize_register($wp_customize) {
            ...
            $wp_customize->add_setting('remove_hamburger_text', array(
                'default'       => false,
                'transport'     => 'refresh',
                'sanitize_callback' => 'oftsCommon::sanitize_boolean'
            ));
            $wp_customize->add_control('remove_hamburger_text', array(
                'label'         => __('Remove Hamburger Text', 'options-for-twenty-seventeen'),
                'description'   => __('Remove the word "Menu" from the mobile menu button.', 'options-for-twenty-seventeen'),
                'section'       => 'ofts_navigation',
                'settings'      => 'remove_hamburger_text',
                'type'          => 'checkbox'
            ));
...
<?php
            ...
            if (get_theme_mod('remove_hamburger_text')) {

                add_filter('gettext', array($this, 'ofts_gettext'), 10, 3);
...
<?php
        ...
        function ofts_social_links_shortcode($atts, $content = null, $tag = '') {
			...
			if (has_nav_menu('social')) {
                ...
                $social_links = '<nav class="social-navigation' . (isset($atts['class']) && sanitize_html_class($atts['class']) ? ' ' . sanitize_html_class($atts['class']) : '') . '" role="navigation" aria-label="' . esc_attr__('Social Links Menu', 'options-for-twenty-seventeen') . '">';
...
<?php
        ...
        public function ofts_gettext($translated_text, $untranslated_text, $domain) {

            if (
                'twentyseventeen' === $domain &&
                'Menu' === $untranslated_text
            ) {
